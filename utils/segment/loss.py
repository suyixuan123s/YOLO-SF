...
from ..loss import FocalLoss, smooth_BCE, QFocalLoss
from ..metrics import bbox_iou, box_iou
from ..torch_utils import de_parallel, is_parallel
...
class VariFocalLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        varifocal_weight = true * (true > 0.0).float() + self.alpha * (pred_prob - true).abs().pow(self.gamma) * (
                    true <= 0.0).float()
